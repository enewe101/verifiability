...
def readData(datafile):
	...
	X_Values = df[:,2:].astype(np.float)

	X_train, X_test, train_metadata, test_metadata = train_test_split(X_Values, metaData, test_size=0.1, random_state=200)
...
def harm_log(X_train, y_train):
	harmonic_logistic = HarmonicLogistic(lengths=[170, 48, 632])
	harmonic_logistic.fit(X_train, y_train, verbose=False)

	harmonic_logisticSK = HarmonicLogisticSK(lengths=[170, 48, 632])
...
def main():
	...
	print "Reading Data"
	X_train, X_test, y_train, y_test, train_meta, test_meta = readData('data/verifiabilityNumFeatures')
	print
	print
	...
	print
	predictions = errorAnal(linRegression, X_train, y_train, X_test, y_test)
	print
	...
	print
	print "Error Analysis"
	predictions = errorAnal(lassoModel, X_train, y_train, X_test, y_test)
	print
	print "Quote Analysis"
	top_bottom_quote_examples(predictions, y_test, test_meta)
	print
	...
	harmonic_logistic, harmonic_logisticSK = harm_log(X_train, y_train)
	print
	print "Error Analysis"
	predictions = harmonic_logistic.predict(X_test)
	print
	...
	predictions = errorAnal(harmonic_logisticSK, X_train, y_train, X_test, y_test)
	print
	print "Quote Analysis"
	top_bottom_quote_examples(predictions, y_test, test_meta)
	print
	...
	print
	print "Error Analysis"
	predictions = errorAnal(best_svr, X_train, y_train, X_test, y_test)
	print
	print "Quote Analysis"
	top_bottom_quote_examples(predictions, y_test, test_meta)
	print
	...
	print
	print "Error Analysis"
	predictions = binnedErrorAnalysis(best_svc, X_train, y_train_binned, X_test, y_test_binned)
	print
	print "Quote Analysis"
	top_bottom_quote_examples(predictions, y_test, test_meta)
	print
